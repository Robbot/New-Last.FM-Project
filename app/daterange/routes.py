diff --git a/app/daterange/routes.py b/app/daterange/routes.py
index 74c91ae8e67a8615ab979a7a7bc11afb71dca6cf..702f62d6e1203ed562dd1d776033839eb60f0777 100644
--- a/app/daterange/routes.py
+++ b/app/daterange/routes.py
@@ -1,30 +1,29 @@
...
-from datetime import datetime, timezone
 ...
 def _build_filters(args) -> tuple[str, list]:
     """
@@ -37,198 +36,199 @@ def _build_filters(args) -> tuple[str, list]:
     """
     where = []
     params: list = []
...
-    # (e.g. "2026-01-06 21:15:00") or similar.
-    # If your column differs, change played_at below.
     rows = conn.execute(
         f"""
         SELECT
...
+          CAST(strftime('%m', {_played_at_expr()}) AS INTEGER) AS month,
           COUNT(*) AS count
         FROM scrobble
-        WHERE CAST(strftime('%Y', played_at) AS INTEGER) = ?
+        WHERE CAST(strftime('%Y', {_played_at_expr()}) AS INTEGER) = ?
 ...
 def days():
     ...
     rows = conn.execute(
         f"""
         SELECT
-          CAST(strftime('%d', played_at) AS INTEGER) AS day,
+          CAST(strftime('%d', {_played_at_expr()}) AS INTEGER) AS day,
           COUNT(*) AS count
         FROM scrobble
-        WHERE CAST(strftime('%Y', played_at) AS INTEGER) = ?
-          AND CAST(strftime('%m', played_at) AS INTEGER) = ?
+        WHERE CAST(strftime('%Y', {_played_at_expr()}) AS INTEGER) = ?
...
+        WHERE {_played_at_expr()} BETWEEN ? AND ?
     ...
     rows = conn.execute(
         f"""
-        SELECT played_at, artist, album, track
